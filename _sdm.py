...
class Esdm(minimalmodbus.Instrument):
    """docstring for sdm"""

    def __init__(self, arg=None):
        self.__conf = arg if arg else DEFAULT_CFG
        try:
            minimalmodbus.Instrument.__init__(self, self.get_cfg('port'), self.get_cfg('slaveaddress'))
    ...
    def get_str(self, nparam=0, sfmt=False):
        _reg = SDM230[nparam]
        if sfmt:
            fmt = '[{0}] {1} {2:' + _reg[_FMT] + '} {3}'
        else:
            fmt = '{0:2} {1:>42} {2:' + _reg[_FMT] + '} {3}'
        # print fmt
        return fmt.format(nparam, _reg[_DESC], self.get(nparam), _reg[_SYMBOL])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--port', help='Default: ' + str(DEFAULT_CFG['port']))
    ...
    if args.port:
        DEFAULT_CFG['port'] = args.port
    if args.baudrate:
        DEFAULT_CFG['baudrate'] = int(args.baudrate)
    if args.slaveaddress:
